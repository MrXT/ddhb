...
public class AlipayController {
    ...
    @Autowired
    private FmWtOrderformService orderformService;
    ...
    public void tradeNodify(HttpServletRequest request,HttpServletResponse response) throws IOException {
        ...
        //商户订单号
        String id = params.get("out_trade_no");//就是数据表wt_orderform  `ID` varchar(36) NOT NULL COMMENT '订单编号',
        //支付宝交易号
        ...
        String watchParam = ("buyerEmail="+buyerEmail+" tradeCode="+tradeCode+" tradeStatus="+tradeStatus);
        if(AlipayNotify.verifyTrade(params)){//验证支付宝发出的交易消息的合法性
            if ("TRADE_SUCCESS".equals(tradeStatus)){
                logger.debug("交易成功！"+watchParam);
                //TODO 修改orderform支付状态、支付账号、支付时间、交易号
                orderformService.doUpdateOrderStatus(id, tradeCode, buyerEmail, tradeStatus);
            }else{
