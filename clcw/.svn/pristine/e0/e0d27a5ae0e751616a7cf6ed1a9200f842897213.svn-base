...
<script type="text/javascript">
	...
	var createWin = {
		addLine : function() {
			...
			return "<div class='addLine'><div class='first-column'><input type='text' id='departure' name='departure' placeholder='起点（拼音支持首字母输入）' code='0' /><input type='text' id='destination' name='destination' placeholder='终点（拼音支持首字母输入）' code='0' /><input type='text' id='tprice' name='tprice' placeholder='票价（元）' class='price' maxlength='4' /></div><div class='second-column'><span></span><input type='button' id='saveLine' name='saveLine' value='保存' onclick='createWin.saveLine();' /></div><div class='city-div'></div></div>";
		},
		saveLine : function() {
			error_result = "";
			if (Tools.CheckInputBox($("#departure")) == 0) {
				error_result = "请输入并选择线路起点城市！";
			} else if (Tools.CheckInputBox($("#destination")) == 0) {
				error_result = "请输入并选择线路终点城市！";
			} else if ($("#departure").attr("code") == $("#destination").attr("code")) {
				error_result = "线路起点和终点不能相同，请重选！";
				...
				if (!Tools.CheckDigit($("#tprice").val()) || $("#tprice").val() < 1 || $("#tprice").val() > 1000) {
					...
					$.ajax({
						...
						data : {
							departure : $("#departure").attr("code"),
							destination : $("#destination").attr("code"),
					...
					success : function(resultData) {
						...
						for (var i = 0; i < data.length; i++) {
							result_str += "<tr><td class=\"xl-left-td\"><a href=\"javascript:void(0);\" lineid_a='"
									...
									+ "' title='删除线路'></a></td><td class=\"xl-middle-td\"><table cellpadding='0' cellspacing='0' width=\"100%\" class=\"innerTable\"><tr><td>"
									+ data[i]["departurename"]
									+ "—"
