...
.addClassLine .first-column, .addClassLine .second-column {
	height: 60px;
	width: 100%;
}
...
<script type="text/javascript">
	...
	$(function() {
		...
		if ($("#ssite").val() != "" && $("#esite").val() != "") {
			...
			for (var i = 0; i < _start.length; i++) {
				createInputBox(i, $("#start"), "text", _start[i].split(",")[0]);
				$("#start" + i).val(_start[i].split(",")[1]);
			}
			for (var i = _start.length; i < 12; i++) {
				createInputBox(i, $("#start"), "text", "");
			}
			createInputBox("start", $("#start"), "a", "");

			for (var i = 0; i < _end.length; i++) {
				createInputBox(i, $("#end"), "text", _end[i].split(",")[0]);
				$("#end" + i).val(_end[i].split(",")[1]);
			}
			for (var i = _end.length; i < 12; i++) {
				createInputBox(i, $("#end"), "text", "");
			}
			createInputBox("end", $("#end"), "a", "");
			$("#addBX").show();
		} else {
			for (var i = 0; i < 12; i++) {
				createInputBox(i, $("#start"), "text", "");
	...
	function createInputBox(id, obj, type, tag) {
		...
		if (type == "text") {
			elements = "<input type='text' id='" + obj.attr("id") + id + "' name='" + obj.attr("id") + id + "' tag='" + tag
					+ "' onmouseout='checkValue(this);'  title='点击设置途径站'>";
		} else {
	...
	var createWin = {
		...
		addClassLine : function() {
			setTimeout(function() {
				...
				createWin.initZhanDian($("#esite"), "end");
			}, 200);
			return "<div class='addClassLine'><div class='first-column'><div class='name'>班线名：</div><input type='text' id='clname' name='clname' placeholder='例如“成乐1号线”' /></div><div class='second-column'><div class='name'>　途径：</div><div class='zdSelect'><div id='zdian' name='zdian'></div><span title='点击增加站点' class='addTJ' onclick='createWin.createTuJing()'></span></div></div><div class='zdList'><div class='start'><div class='zd-title'></div></div><div class='end'><div class='zd-title'></div></div></div><div class='third-column'><span></span><input type='button' id='saveClassLine' name='saveClassLine' value='保存' flag='0' onclick='createWin.saveClassLine($(this).attr(\"flag\"));' hidefocus=\"hidefocus\" /></div></div>";
		},
		initTuJing : function(content) {
			if (content == 0) {
				for (var i = 0; i < tu_jing; i++) {
					$(
		...
		bindTuJingEvent : function(id) {
			...
			for (var i = 0; i < tujing_zd_columns.length; i++) {
				...
				tujing_closer_element.bind("click", function() {
					if ($(".zdSelect .zhandian-columns").length > 2) {
						$(this).parent().remove();
					}
				...
				success : function(resultData) {
					...
					for (var i = 0; i < data.length; i++) {
						...
						result_str += "<tr><td class=\"xl-left-td\"><a href=\"javascript:void(0);\" classid='" + data[i]["classlineId"]
								...
								+ "' title='删除班线'></a></td><td style='width:10%;color:#000;font-weight:bold;' id='cline_" + data[i]["classlineId"]
								+ "'>" + data[i]["classlineName"]
								+ "</td><td id='sn_" + data[i]["classlineId"] + "' style='text-align:left;' classline=" + data[i]["siteName1"] + ">"
								+ siteName + "</td><td class=\"edit\"><a href=\"javascript:void(0);\" onclick=\"createWin.editClassLine($('#sn_"
		...
		editClassLine : function(classLine, classLineName) {
			kyWin(createWin.addClassLine(), 1000, 350, '编辑班线', true, true);
		...
		saveClassLine : function(flag) {
			...
			if (j >= 2) {
				...
				if (isStartPass && isEndPass && isFirstStartPass) {
					...
					if (Tools.CheckInputBox($("#clname")) == 0) {
						error_result = "请输入班线名！";
	...
	function checkValue(ob) {
		var inputs = $(ob).parent().find("input[type='text']");
		var pass = true;
		$.each(inputs, function(i, input) {
			if (ob != input && $(input).val() != '' && ob.value == $(input).val()) {
				pass = false;
			}
		});
		if (!pass) {
			...
			<div id="main">
				...
				<div class="data-containers">
					...
					<div class="setZD">
						<div class="zhandian-column">
							<div class="city-name">${line.departurename}—</div>
							<div class="zhandian-box" id="start"></div>
						</div>
						<div class="zhandian-column">
							<div class="city-name">${line.destinationname}—</div>
				...
				<div class="data-containers" style="margin-top: 10px;">
					<div class="data-search">
						<div class="inner">
							<div class="left">
								<span class="name" id="classLine"></span>
							</div>
							<div class="right">
								<input type="button" id="addBX" name="addBX" value="添加班线" class="banxian-add"
									onclick="kyWin(createWin.addClassLine(),1000,350,'添加班线', true, true);$('#clname').attr('placeholder','例如：“'+$('#line_name').val()+'1号线”');Tools.supportPlaceholder();$('#saveClassLine').attr('flag', 0);createWin.initTuJing(0);$('.zdList .start .zd-title').text($('#line_start').val()+'站点：');$('.zdList .end .zd-title').text($('#line_end').val()+'站点：')"
									hidefocus="hidefocus" />
							</div>
						</div>
					</div>
